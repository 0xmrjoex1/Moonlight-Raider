...
class Render:
    ...
    def render_ascii(self):
        clear()
        title(f"Moonlight | Connected as {os.getlogin()} | Made By Mr.JoEx")
        edges = ["╗", "║", "╚", "╝", "═", "╔"]
        ascii = f"""
{'███╗░░░███╗░█████╗░░█████╗░███╗░░██╗██╗░░░░░██╗░██████╗░██╗░░██╗████████╗'.center(self.size)}
{'████╗░████║██╔══██╗██╔══██╗████╗░██║██║░░░░░██║██╔════╝░██║░░██║╚══██╔══╝'.center(self.size)}
...
"""
        for edge in edges:
            title = title.replace(edge, f"{self.background}{edge}{C['red']}")
        print(title)
...
class Menu:
    ...
    def main_menu(self):
        ...
        if check(base64.b64decode("TW9vbmxpZ2h0").decode("utf-8")) and check(base64.b64decode("TWFkZSBCeSBNci5Kb0V4").decode("utf-8")):
            pass
        else:
